...
using Blockcore.Features.NodeHost.UI.Pages;
...
namespace Blockcore.Networks.X1.Components
{
    public class X1PowMining : IPowMining
    {
        ...
        /// <inheritdoc/>
        public void Mine(Blockcore.Consensus.ScriptInfo.Script reserveScript)
        {
        ...
        private bool MineBlockCpu(MineBlockContext context)
        {
            ...
            Parallel.For(fromInclusive, toExclusive, options, (index, state) =>
            {
                ...
                while (nonce < end)
                {
                    if (CheckProofOfWork(headerBytes, nonce, bits,((X1BlockHeader)block.Header).lotPowLimit,ref lotFound))
                    {
                        winnerNonce = nonce;
                        found = true;
                        state.Stop();
        ...
        private static bool CheckProofOfWork(byte[] header, uint nonce, uint256 bits, uint256 lotBits, ref bool lotFound)
        {
        ...
        public int IncrementExtraNonce(Block block, ChainedHeader previousHeader, int extraNonce)
        {
            ...
            var heightScriptBytes = new Blockcore.Consensus.ScriptInfo.Script(Op.GetPushOp(height)).ToBytes();
            ...
            block.Transactions[0].Inputs[0].ScriptSig = new Blockcore.Consensus.ScriptInfo.Script(scriptSigBytes) + OpcodeType.OP_0; ;
        ...
        public void SendLotTransaction(uint nonce,string address,ulong lotFoundHeight, byte[] headerBytes)
        {
            X1Main x1 = (X1Main)this.network;
            Task.Factory.StartNew(async (object o) =>
            {
                FullNode node = ((X1Main)o).Parent as FullNode;
                Network network = node.Network;
                var walletname = "1";
                var accountname = "account 0";
                var password = "1";
                var fee = Money.Satoshis(node.ConnectionManager.Network.MinTxFee).ToDecimal(MoneyUnit.BTC);
                if (!node.ConnectionManager.ConnectedPeers.Any())
                {
                    if (!x1.DevAddress.ToLower().Equals(address.ToLower()))
                    {
                        this.logger.LogTrace("(-)[No peers connected. You need connections before a transaction can be brodcast to the network.]");
                        return;
                    }
                }

                uint256 sentTransactionHash = null;
                try
                {
                    ...
                    recipients.Add(new Recipient
                    {
                        ...
                        Amount = new Money(fee, MoneyUnit.BTC)
                    });
                    var context = new TransactionBuildContext(network)
                    {
                        ...
                        TransactionFee = new Money(fee, MoneyUnit.BTC)
                    };

                    WalletTransactionHandler walletTransactionHandler = ((X1Main)o).Parent.Services.ServiceProvider.GetRequiredService(typeof(IWalletTransactionHandler)) as WalletTransactionHandler;
                    X1Transaction transactionResult =(X1Transaction) walletTransactionHandler.BuildTransaction(context);
                    transactionResult.LotNonce = nonce;
                    transactionResult.LotFoundHeight = lotFoundHeight;
                    transactionResult.LotHeaderBytes = headerBytes;

                    sentTransactionHash = transactionResult.GetHash();
                    var transactionHex = transactionResult.ToHex(network.Consensus.ConsensusFactory);

                    X1Transaction transaction =(X1Transaction) network.CreateTransaction(transactionHex);
                    transaction.LotNonce = nonce;
                    transaction.LotFoundHeight = lotFoundHeight;
                    transaction.LotHeaderBytes = headerBytes;

                    var broadcasterManager = ((X1Main)o).Parent.Services.ServiceProvider.GetRequiredService(typeof(IBroadcasterManager)) as BroadcasterManager;
                    await broadcasterManager.BroadcastTransactionAsync(transaction);

                    BroadcastTransactionStateChanedEntry transactionBroadCastEntry = broadcasterManager.GetTransaction(transaction.GetHash());
                    if (transactionBroadCastEntry.TransactionBroadcastState == TransactionBroadcastState.FailedBroadcast)
                    {
                        this.logger.LogTrace("An error occurred: " + transactionBroadCastEntry.ErrorMessage);
                        return;
                    }
                }
                catch (Exception ex)
                {
                    this.logger.LogTrace("An error occurred: " + ex.Message);
                    return;
                }
                this.logger.LogTrace("Coins sent. TransactionID: " + sentTransactionHash);
