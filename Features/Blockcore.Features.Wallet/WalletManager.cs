...
namespace Blockcore.Features.Wallet
{
    ...
    public class WalletManager : IWalletManager
    {
        ...
        public long GetMinedAddressBalance(string address)
        {
            Types.Wallet wallet = this.GetFirstWallet();
            UnspentOutputReference[] res = null;
            lock (this.lockObject)
            {
                res = wallet.GetAllAddressSpendableTransactions(wallet.walletStore, this.ChainIndexer.Tip.Height,address, 0, null).ToArray();
                // Get total spendable balance in the account.
                long balance = res.Sum(t => t.Transaction.Amount);
                return balance;
            }
        }
        ...
        public Types.Wallet GetFirstWallet()
        {
