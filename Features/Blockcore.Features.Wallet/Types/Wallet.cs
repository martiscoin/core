...
namespace Blockcore.Features.Wallet.Types
{
    ...
    public class Wallet
    {
        ...
        public IEnumerable<UnspentOutputReference> GetAllAddressSpendableTransactions(IWalletStore walletStore, int currentChainHeight,string address, int confirmations = 0, Func<HdAccount, bool> accountFilter = null)
        {
            IEnumerable<HdAccount> accounts = this.GetAccounts(accountFilter);

            return accounts.SelectMany(x => x.GetAddressSpendableTransactions(walletStore, currentChainHeight, this.Network.Consensus.CoinbaseMaturity, address, confirmations));
        }
    ...
    public class HdAccount
    {
        ...
        /// <summary>
        /// Lists all spendable transactions in an address.
        /// </summary>
        ...
        /// <returns>A collection of spendable outputs that belong to the given address.</returns>
        ...
        public IEnumerable<UnspentOutputReference> GetAddressSpendableTransactions(IWalletStore walletStore, int currentChainHeight, long coinbaseMaturity, string address, int confirmations = 0)
        {
            int countFrom = currentChainHeight + 1;
            foreach (TransactionOutputData transactionData in walletStore.GetUnspentForAddress(address))
            {
                ...
                {
                    HdAddress unspentOutputReferenceAddress = new HdAddress();
                    unspentOutputReferenceAddress.Address = address;
                    yield return new UnspentOutputReference
                    {
                        Account = null,
                        Address = unspentOutputReferenceAddress,
